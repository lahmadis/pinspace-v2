...
export default function NetworkRouteShell({
  ...
  if (loadState === 'error' || loadState === 'not-found') {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-pinspace-forest px-4">
        <StatusState
          status="error"
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-pinspace-forest text-white">
      <header ref={headerRef} className="fixed inset-x-0 top-0 z-40 border-b border-white/15 bg-pinspace-forest/95 backdrop-blur-md">
        <div className="flex min-h-20 flex-wrap items-center gap-3 px-4 py-3 sm:px-6">
          <Link href={backHref} className="inline-flex min-h-11 items-center gap-2 rounded-pinspace px-3 text-sm font-semibold text-white hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><ArrowLeft className="h-4 w-4" aria-hidden="true" />{backLabel}</Link>
