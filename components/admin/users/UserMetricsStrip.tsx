'use client'

import { GraduationCap, UserCheck, Users } from 'lucide-react'
import { Card, MetricsSkeletonGrid } from '@/components/ui'
import type { AdminUser } from '@/types/admin'

function MetricCard({ label, value, hint, icon: Icon }: { label: string; value: number; hint: string; icon: any }) {
  return (
    <Card className="p-4 flex items-start justify-between">
      <div>
        <p className="text-xs font-semibold text-text-dim uppercase tracking-wider">{label}</p>
        <p className="text-2xl font-bold text-text-primary tabular-nums mt-1">{value}</p>
        <p className="text-xs text-text-secondary mt-1">{hint}</p>
      </div>
      <div className="p-2 bg-primary-muted rounded text-accent">
        <Icon className="w-4.5 h-4.5" />
      </div>
    </Card>
  )
}

export interface UserMetricsStripProps {
  users: AdminUser[]
  loading: boolean
}

export function UserMetricsStrip({ users, loading }: UserMetricsStripProps) {
  if (loading) {
    return (
      <div className="mb-6">
        <MetricsSkeletonGrid count={3} />
      </div>
    )
  }

  const total = users.length
  const instructors = users.filter((u) => u.accountRole === 'instructor').length
  const students = total - instructors

  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
      <MetricCard label="Total Registered Users" value={total} hint="All platform accounts" icon={Users} />
      <MetricCard label="Student Accounts" value={students} hint="Student user role" icon={GraduationCap} />
      <MetricCard label="Instructor Accounts" value={instructors} hint="Instructor user role" icon={UserCheck} />
    </div>
  )
}

export default UserMetricsStrip
