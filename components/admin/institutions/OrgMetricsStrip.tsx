'use client'

import { Building2, GraduationCap, Network } from 'lucide-react'
import { Card, MetricsSkeletonGrid } from '@/components/ui'
import type { InstitutionWithCount } from '@/types/admin'

function MetricCard({ label, value, hint, icon: Icon }: { label: string; value: number; hint: string; icon: any }) {
  return (
    <Card className="p-4 flex items-start justify-between">
      <div>
        <p className="text-xs font-semibold text-text-dim uppercase tracking-wider">{label}</p>
        <p className="text-2xl font-bold text-text-primary tabular-nums mt-1">{value}</p>
        <p className="text-xs text-text-secondary mt-1">{hint}</p>
      </div>
      <div className="p-2 bg-primary-muted rounded text-accent">
        <Icon className="w-4.5 h-4.5" />
      </div>
    </Card>
  )
}

export interface OrgMetricsStripProps {
  institutions: InstitutionWithCount[]
  loading: boolean
}

export function OrgMetricsStrip({ institutions, loading }: OrgMetricsStripProps) {
  if (loading) {
    return (
      <div className="mb-6">
        <MetricsSkeletonGrid count={3} />
      </div>
    )
  }

  const total = institutions.length
  const schools = institutions.filter((i) => (i.type ?? 'university') === 'university').length
  const firms = institutions.filter((i) => i.type === 'firm').length

  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
      <MetricCard label="Total Organizations" value={total} hint="Schools & registered design firms" icon={Network} />
      <MetricCard label="Universities & Schools" value={schools} hint="Academic institutions" icon={GraduationCap} />
      <MetricCard label="Design Firms" value={firms} hint="Professional studio teams" icon={Building2} />
    </div>
  )
}

export default OrgMetricsStrip
