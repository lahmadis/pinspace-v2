 ...
 *
 * Selection is the only place yellow appears here: a left border and the row
 * number chip. Everything else stays ink on paper so the sheets keep priority.
 */
  ...
  return (
    ...
    >
      <button
        ...
        style={{ color: ROOM.wall, borderBottom: `1px solid ${ROOM.hairline}` }}
        aria-expanded={!collapsed}
      >
        <span
          className="text-[10px] uppercase tracking-[0.18em] truncate"
          style={{ fontFamily: MONO_STACK, color: ROOM.wall }}
        >
          {collapsed ? String(students.length).padStart(2, '0') : `Roster · ${String(students.length).padStart(2, '0')}`}
        </span>
        <svg
          width="14" height="14" viewBox="0 0 24 24" fill="none"
          stroke={ROOM.wall} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
              ...
              <li key={student.id}>
                <button
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${isSelected ? ROOM.yellow : 'transparent'}`,
                    background: isSelected ? 'rgba(255,200,0,0.10)' : 'transparent',
                  }}
                >
                  <span
                    className="w-6 shrink-0 text-[10px] tabular-nums"
                    style={{ fontFamily: MONO_STACK, color: isSelected ? ROOM.yellow : ROOM.hairline }}
                  >
                  ...
                  <span
                    ...
                    style={{
                      background: isSelected ? ROOM.yellow : ROOM.green,
                      color: isSelected ? ROOM.ink : ROOM.wall,
                    }}
                    aria-hidden="true"
                  >
                    {student.initials}
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[13px] font-semibold" style={{ color: ROOM.wall }}>
                      {student.name}
                    </span>
                    <span
