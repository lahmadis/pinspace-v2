...
export default function QuickNotePanel({ boardTitle, onAddNote }: QuickNotePanelProps) {
  ...
  return (
    <motion.aside
      ...
      className="fixed inset-x-[max(1rem,env(safe-area-inset-left))] bottom-[max(1rem,env(safe-area-inset-bottom))] z-40 max-h-[min(70dvh,30rem)] overflow-y-auto rounded-pinspace-lg border border-border bg-background-light p-4 text-text-primary shadow-[var(--shadow-raised)] motion-reduce:transition-none sm:left-auto sm:right-[max(1rem,env(safe-area-inset-right))] sm:w-96"
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        {!author && (
          <>
            ...
            <input
              ...
              className="min-h-11 w-full rounded-pinspace border border-border bg-background-light px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              autoFocus
            />
          </>
        )}
        ...
        <textarea
          ...
          className="w-full resize-y rounded-pinspace border border-border bg-background-light px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:bg-background-lighter"
          rows={3}
          disabled={!author}
        />
        ...
        <div className="flex gap-2">
          <button
            ...
            className="min-h-11 flex-1 rounded-pinspace border border-pinspace-ink bg-primary px-4 py-2 text-sm font-semibold text-pinspace-ink shadow-[0_3px_0_rgb(var(--color-ink))] hover:bg-primary-light focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:cursor-not-allowed disabled:opacity-55"
          >
